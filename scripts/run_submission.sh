...
./build/${SUBFOLDER}/${TARGET_NAME} |& tee output_user.txt &

HELLO_PID=$(pgrep -f ${TARGET_NAME})
ps -F -p ${HELLO_PID} |& tee -a output_user.txt
# after 4 seconds, signal will be sent anyways
read -t 4 -p "Press any key to send TERM signal to PID ${HELLO_PID} ..."
kill -s TERM ${HELLO_PID}
echo "Signalled."

sudo ./build/${SUBFOLDER}/${TARGET_NAME} | tee output_root.txt
